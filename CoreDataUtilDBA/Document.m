...
- (void)windowControllerDidLoadNib:(NSWindowController *)aController
{
    ...
    if (self.fileURL) {
        ...
        NSManagedObject *modelObject = models[0];
        NSURL *modelURL = [NSURL fileURLWithPath:[modelObject valueForKey:@"path"]];
        
        
        request = [[NSFetchRequest alloc] init];
        entity = [NSEntityDescription entityForName:@"Storage"
                             inManagedObjectContext:self.managedObjectContext];
        [request setEntity:entity];
        
        NSArray *storages = [self.managedObjectContext executeFetchRequest:request error:nil];
        NSManagedObject *storageObject = storages[0];
        NSURL *storageURL = [NSURL fileURLWithPath:[storageObject valueForKey:@"path"]];
        NSNumber *format = [storageObject valueForKey:@"type"];
        
        [self openFiles:modelURL persistenceFile:storageURL persistenceType:format.integerValue];
...
- (void)openFileSheetController:(OpenFileSheetController *)controller didSelectModelURL:(NSURL *)modelURL storageURL:(NSURL *)storageURL persistFileFormat:(NSUInteger)format {
    [self closeOpenFileSheetController];
    ...
    [self openFiles:modelURL persistenceFile:storageURL persistenceType:format];
    
    self.documentView.frame = self.view.bounds;
    [self.view addSubview:self.documentView];
    
    
    NSEntityDescription *modelEntity = [NSEntityDescription entityForName:@"Model"
                                                   inManagedObjectContext:self.managedObjectContext];
    NSEntityDescription *storageEntity = [NSEntityDescription entityForName:@"Storage"
                                                     inManagedObjectContext:self.managedObjectContext];
    
    NSManagedObject *modelObject = [[NSManagedObject alloc] initWithEntity:modelEntity
                                            insertIntoManagedObjectContext:self.managedObjectContext];
    [modelObject setValue:modelURL.path forKey:@"path"];
    
    NSManagedObject *storageObject = [[NSManagedObject alloc] initWithEntity:storageEntity
                                            insertIntoManagedObjectContext:self.managedObjectContext];
    [storageObject setValue:storageURL.path forKey:@"path"];
    [storageObject setValue:@(format) forKey:@"type"];
}
...
- (void)getInfoAction
{
    ...
    [infoSheetController show:self.windowForSheet :entityDescription];
}
...
- (void)getFetchRequestInfoAction
{
	...
	[fetchRequestController show:self.windowForSheet forFetchRequest:fetchRequest title:[self.coreDataIntrospection fetchRequestAtIndex:selected]];
}
